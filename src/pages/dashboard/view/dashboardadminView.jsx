...
import { FaGlobe, FaArrowUp, FaArrowDown, FaBriefcase } from "react-icons/fa";
import { PiChartLineUp, PiChartBar, PiChartPieSlice, PiCalendarBlank, PiCaretDown, PiX, PiPencilSimple } from "react-icons/pi";

// ─── DONNÉES ──────────────────────────────────────────────────────────────────

const visibiliteGrowthData = [
  { month: "Août", visites: 2450 },
  { month: "Sep", visites: 2890 },
  { month: "Oct", visites: 3420 },
  { month: "Nov", visites: 3980 },
  { month: "Déc", visites: 4250 },
  { month: "Jan", visites: 5120 },
  { month: "Fév", visites: 5890 },
];
...
const metiersRecherchesData = [
  { name: "Développeur Full Stack", value: 450, croissance: "+15%" },
  { name: "Data Scientist", value: 380, croissance: "+22%" },
  { name: "DevOps Engineer", value: 320, croissance: "+18%" },
  { name: "Cloud Architect", value: 290, croissance: "+25%" },
  { name: "Cybersecurity Analyst", value: 260, croissance: "+30%" },
  { name: "Product Manager", value: 210, croissance: "+12%" },
];

// Remplacement du violet (#6366f1) par du bleu (#3b82f6)
...
function StatCardModerne({ label, value, icon: Icon, trend, trendLabel, accentColor, suffix = "visites" }) {
      ...
      <div className="flex items-end gap-1 mt-6 h-6">
        {[30, 45, 38, 52, 48, 65, 78].map((h, i) => (
          <div key={i} className="flex-1 flex items-end h-full">
            <div
              className={`w-full rounded-t-sm transition-all duration-500 ${i > 4 ? 'opacity-100' : 'opacity-20'}`}
              style={{
                height: `${h}%`,
...
function LineChartSVG() {
  ...
  const pts = visibiliteGrowthData.map((d) => ({ y: d.month, v: d.visites }));

  const W = 580, H = 230;
  const PL = 46, PR = 22, PT = 22, PB = 30;
  const cW = W - PL - PR, cH = H - PT - PB;
  const minV = 2000, maxV = 6200;
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="mb-6">
        <h3 className="text-sm font-bold text-gray-900">Croissance des visibilités</h3>
        <p className="text-xs text-gray-500">Évolution mensuelle des visites sur la plateforme</p>
      </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto overflow-visible">
        ...
        {[2500, 3500, 4500, 5500].map((v) => (
          <g key={v}>
            <line x1={PL} y1={yP(v)} x2={W - PR} y2={yP(v)} className="stroke-gray-100" strokeWidth="1" strokeDasharray="4 4" />
            <text x={PL - 10} y={yP(v) + 4} textAnchor="end" className="text-[10px] font-medium fill-gray-400">
        ...
        <path d={lp} fill="none" stroke="#3b82f6" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />

        {pts.map((d, i) => (
          <g key={i} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} className="cursor-pointer group">
            <line x1={xP(i)} y1={PT} x2={xP(i)} y2={PT + cH} stroke="#3b82f6" strokeWidth="1" strokeDasharray="4 4" className={`transition-opacity duration-300 ${hov === i ? 'opacity-30' : 'opacity-0'}`} />
            <circle cx={xP(i)} cy={yP(d.v)} r={hov === i ? 6 : 4} className={`transition-all duration-300 fill-white ${hov === i ? 'stroke-[3px]' : 'stroke-2'}`} stroke="#3b82f6" />
            {hov === i && (
              <g className="animate-in zoom-in-95 duration-200">
                <rect x={xP(i) - 35} y={yP(d.v) - 40} width="70" height="26" rx="6" className="fill-gray-900 shadow-lg" />
                <polygon points={`${xP(i)-6},${yP(d.v)-14.5} ${xP(i)+6},${yP(d.v)-14.5} ${xP(i)},${yP(d.v)-8}`} className="fill-gray-900" />
                <text x={xP(i)} y={yP(d.v) - 23} textAnchor="middle" className="text-[11px] font-bold fill-white">
                  {d.v.toLocaleString('fr-FR')}
                </text>
              </g>
            )}
            <text x={xP(i)} y={H - 5} textAnchor="middle" className={`text-[11px] font-medium transition-colors ${hov === i ? 'fill-gray-900 font-bold' : 'fill-gray-400'}`}>
              {d.y}
            </text>
          </g>
        ))}
...
function BarChartSVG() {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="mb-6">
        <h3 className="text-sm font-bold text-gray-900">Activité hebdomadaire</h3>
        <p className="text-xs text-gray-500">Vues réparties par jour de la semaine</p>
      </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto overflow-visible">
        ...
        {[500, 1000, 1500, 2000, 2500].map((v) => {
          ...
          return (
            <g key={v}>
              <line x1={PL} y1={y} x2={W - PR} y2={y} className="stroke-gray-100" strokeWidth="1" strokeDasharray="4 4" />
              <text x={PL - 8} y={y + 4} textAnchor="end" className="text-[10px] font-medium fill-gray-400">
                {v >= 1000 ? `${v / 1000}k` : v}
              </text>
            </g>
          );
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} className="cursor-pointer">
              <rect x={x} y={PT} width={w} height={cH} rx="6" className="fill-gray-50" />
              <rect x={x} y={y} width={w} height={bH} rx="6" fill={`url(#bgrad${i})`} className="transition-all duration-300" opacity={hov !== null && hov !== i ? 0.4 : 1} />
              {hov === i && (
                <g className="animate-in zoom-in-95 duration-200">
                  <rect x={x + w / 2 - 36} y={y - 32} width="72" height="24" rx="6" className="fill-gray-900 shadow-lg" />
                  <polygon points={`${x+w/2-6},${y-8.5} ${x+w/2+6},${y-8.5} ${x+w/2},${y-2}`} className="fill-gray-900" />
                  <text x={x + w / 2} y={y - 16} textAnchor="middle" className="text-[11px] font-bold fill-white">
                    {d.v} vues
                  </text>
                </g>
              )}
              <text x={x + w / 2} y={H - 6} textAnchor="middle" className={`text-[11px] font-medium transition-colors ${hov === i ? 'fill-gray-900 font-bold' : 'fill-gray-400'}`}>
                {d.r}
              </text>
            </g>
          );
...
function DonutChartSVG() {
  const [hov, setHov] = useState(null);
  const total = metiersRecherchesData.reduce((s, d) => s + d.value, 0);

  const cx = 120, cy = 120, R = 100, r = 65;
  let currentAngle = -Math.PI / 2;

  const slices = metiersRecherchesData.map((d, i) => {
    const a = (d.value / total) * 2 * Math.PI;
    const x1 = cx + R * Math.cos(currentAngle), y1 = cy + R * Math.sin(currentAngle);
    const x2 = cx + R * Math.cos(currentAngle + a), y2 = cy + R * Math.sin(currentAngle + a);
    const ix1 = cx + r * Math.cos(currentAngle), iy1 = cy + r * Math.sin(currentAngle);
    const ix2 = cx + r * Math.cos(currentAngle + a), iy2 = cy + r * Math.sin(currentAngle + a);
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="mb-6">
        <h3 className="text-sm font-bold text-gray-900">Métiers les plus recherchés</h3>
        <p className="text-xs text-gray-500">Répartition par domaine (tendance du marché)</p>
      </div>

      <div className="flex flex-col items-center gap-8 md:flex-row">
        <div className="relative flex-shrink-0 mx-auto md:mx-0">
          <svg viewBox="0 0 240 240" className="w-[200px] h-[200px] overflow-visible">
            {slices.map((s, i) => (
              <path 
                key={i} d={s.path} fill={s.color}
                className="transition-all duration-300 cursor-pointer stroke-white" strokeWidth="3"
                opacity={hov !== null && hov !== i ? 0.3 : 1}
                style={{ transformOrigin: `${cx}px ${cy}px`, transform: `scale(${hov === i ? 1.04 : 1})` }}
                onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)}
              />
            ))}
            <text x={cx} y={cy - 4} textAnchor="middle" className="text-3xl font-extrabold fill-gray-900">
              {total}
            </text>
            <text x={cx} y={cy + 16} textAnchor="middle" className="text-[11px] font-semibold tracking-wider uppercase fill-gray-400">
              Offres
            </text>
          </svg>
        </div>
        ...
        <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2">
          {slices.map((s, i) => (
            <div 
              key={i} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)}
              className={`flex items-center justify-between p-3 rounded-xl border transition-all duration-300 cursor-default ${
                hov === i ? "bg-white shadow-md border-transparent scale-[1.02]" : "bg-white border-gray-100 hover:bg-gray-50"
              }`}
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className={`w-3 h-3 rounded-full flex-shrink-0 transition-transform ${hov === i ? 'scale-125 shadow-sm' : ''}`} style={{ backgroundColor: s.color }} />
                <span className={`text-xs font-medium truncate ${hov === i ? 'text-gray-900' : 'text-gray-500'}`} title={s.name}>{s.name}</span>
              </div>
              <div className="flex flex-col items-end flex-shrink-0 pl-2">
                <span className="text-sm font-bold text-gray-900">{s.value}</span>
                <span className="text-[10px] font-bold text-emerald-500">{s.croissance}</span>
              </div>
            </div>
          ))}
        </div>
...
const DashboardAdminView = () => {
  ...
  // États pour la Modal et les Dates
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [tempStartDate, setTempStartDate] = useState("");
  const [tempEndDate, setTempEndDate] = useState("");
  
  // Dates officiellement validées
  ...
  const topMetier = metiersRecherchesData[0];
  const statMetier = {
    label: `Top Métier : ${topMetier.name}`, 
    value: topMetier.value, 
    icon: FaBriefcase, 
    trend: parseInt(topMetier.croissance.replace('+', '').replace('%', '')),
    trendLabel: "demande en hausse", 
    accentColor: "#3b82f6", // Remplace l'indigo par le bleu ici aussi
    suffix: "offres"
  };
  ...
  // Appliquer la date personnalisée depuis la Modal
  ...
  // Texte d'affichage de la période custom dans le select
  const getCustomLabel = () => {
    if (customStartDate && customEndDate) {
      return `${formatDateToFR(customStartDate)} au ${formatDateToFR(customEndDate)}`;
    }
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-white font-sans text-gray-900 transition-colors duration-300">
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        {/* En-tête de la page avec Filtre Avancé */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-2">
            <div className="relative inline-block w-full sm:w-auto group z-10">
              ...
              >
                ...
                <option value="custom" className="font-bold text-blue-600">
                  {dateFilter === "custom" ? getCustomLabel() : "Période personnalisée..."}
                </option>
              </select>
            ...
            {/* Bouton d'édition rapide si Custom est actif */}
            {dateFilter === "custom" && (
              <button 
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 md:p-6 border-b border-gray-100">
            ...
            <div className="flex p-1 bg-white border border-gray-100 shadow-sm rounded-xl">
              {CHART_TABS.map(({ k, I, l }) => (
                ...
                >
                  <I className={`w-4 h-4 ${chart === k ? 'animate-pulse' : ''}`} />
                  <span>{l}</span>
                </button>
              ))}
            </div>
